...
bool CAutowall::HandleBulletPenetration( CWeaponData *wpn_data, FireBulletData &data )
{
	...
	data.current_damage *= pow( wpn_data->m_flRangeModifier, data.trace_length * 0.002f );
	...
	float v40 = v34 * 3.f * fmaxf( 0.f, 3.f / wpn_data->m_flPenetration * 1.25f ) + data.current_damage * final_damage_modifier + v39 * v39 * v34 / 24.f;
...
bool CAutowall::SimulateFireBullet( CBaseEntity *local, CBaseEntity *player, CBaseCombatWeapon *weapon, FireBulletData &data, int spot )
{
	...
	data.current_damage = wpn_data->m_iDamage;
